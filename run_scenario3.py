...
from src.gformula_IPW_parametric import g_formula_parametric, true_value_scenario_2,IPW_parametric
...
results_11 = {key: {s : {"G_formula" : [], "IPW" : [],  "IPW_xgboost" : []} for s in ["s0", "s1", "s2"]} for key in true_res_nde_0}
results_nie_1 = {key: {s : {"G_formula" : [], "IPW" : [],  "IPW_xgboost" : []} for s in ["s0", "s1", "s2"]} for key in true_res_nde_0}
results_10 = {key: {s : {"G_formula" : [], "IPW" : [],  "IPW_xgboost" : []} for s in ["s0", "s1", "s2"]} for key in true_res_nde_0}
results_01 = {key: {s : {"G_formula" : [], "IPW" : [],  "IPW_xgboost" : []} for s in ["s0", "s1", "s2"]} for key in true_res_nde_0}
results_00 ={key: {s : {"G_formula" : [], "IPW" : [],  "IPW_xgboost" : []} for s in ["s0", "s1", "s2"]} for key in true_res_nde_0}
results_nde_0 = {key: {s : {"G_formula" : [], "IPW" : [],  "IPW_xgboost" : []} for s in ["s0", "s1", "s2"]} for key in true_res_nde_0}
results_nie_0 = {key: {s : {"G_formula" : [], "IPW" : [],  "IPW_xgboost" : []} for s in ["s0", "s1", "s2"]} for key in true_res_nde_0}
results_nde_1 = {key: {s : {"G_formula" : [], "IPW" : [],  "IPW_xgboost" : []} for s in ["s0", "s1", "s2"]} for key in true_res_nde_0}
...
for i in range(n_sim):
    ...
    for k, p in itertools.product(range(3), repeat=2):
        for j in range(3):
            ...
            results_10[key][s]["IPW"].append(IPW_parametric(j,k,p, 1, 0, data_all))
            results_11[key][s]["IPW"].append(IPW_parametric(j,k,p, 1, 1, data_all))

            results_01[key][s]["IPW"].append(IPW_parametric(j,k,p, 0, 1, data_all))
            results_00[key][s]["IPW"].append(IPW_parametric(j,k,p, 0, 0, data_all))
            ...
            results_10[key][s]["IPW_xgboost"].append(IPW_parametric(j,k,p, 1, 0, data_all,population_algo ="xgboost"))
            results_11[key][s]["IPW_xgboost"].append(IPW_parametric(j,k,p, 1, 1, data_all,population_algo = "xgboost"))

            results_01[key][s]["IPW_xgboost"].append(IPW_parametric(j,k,p, 0, 1, data_all,population_algo = "xgboost"))
            results_00[key][s]["IPW_xgboost"].append(IPW_parametric(j,k,p, 0, 0, data_all,population_algo = "xgboost"))

            results_nde_0[key][s]["IPW_xgboost"].append(results_10[key][s]["IPW_xgboost"][i]-results_00[key][s]["IPW_xgboost"][i])
        
            results_nie_1[key][s]["IPW_xgboost"].append(results_11[key][s]["IPW_xgboost"][i]-results_10[key][s]["IPW_xgboost"][i])

            results_nde_1[key][s]["IPW_xgboost"].append(results_11[key][s]["IPW_xgboost"][i]-results_01[key][s]["IPW_xgboost"][i])
        
            results_nie_0[key][s]["IPW_xgboost"].append(results_01[key][s]["IPW_xgboost"][i]-results_00[key][s]["IPW_xgboost"][i])
